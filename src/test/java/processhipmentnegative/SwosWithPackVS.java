...
public class SwosWithPackVS {
	...
	public static String NewGroupcode="SWOS1";
	public static String ShipVia="FSMS(MPS:YES)_GN";
	private boolean flag = false; // Instance variable
	...
	public void Groups() throws InterruptedException {
        ...
        Thread.sleep(3000);
     // Handle error message if it appears
        WebElement errorMessage = driver.findElement(By.xpath("//span[@id='errorMsg']"));
        if (errorMessage.isDisplayed()) {
            String errorText = errorMessage.getText();
            if (errorText.equals("Entry already exists in the database. Please try again.")) {
            	 flag = true;
                driver.findElement(By.id("btnErrorBoxOk")).click();
                WebElement customerCodeNew = driver.findElement(By.id("SWOSFrmtxtCustomerCodePS"));
                customerCodeNew.clear();
                customerCodeNew.sendKeys(NewGroupcode);
                okButton.click();
                Thread.sleep(2000);
	...
	public void CheckGroups() {
        ...
        Thread.sleep(3000);
	          ...
	          List < WebElement> codeTable = driver.findElements(By.xpath("//table[@id='tblSWOSGroupFormListPS']//td[1]"));
	            boolean codeFound = false;
	            // Loop through each row in the table
	            for (WebElement row : codeTable) {
	                // Check if the row contains the code
	                if (row.getText().contains(Custcode)) {
	                    codeFound = true;
	                    // Click the "OK" button
	                    row.click();
	                    WebElement okButton = row.findElement(By.id("btnOkButtonSGFPS"));
	                    okButton.click();
	                    break;
	                }
	            }
	...
	public void selecshipvia() throws InterruptedException {
		 ...
		 Thread.sleep(3000);
	        WebElement shipviaSearch = driver.findElement(By.xpath("//*[@id=\"card4\"]/div[2]/form/div[1]/div[3]/span")); // Search the																								// Shipvias list
			shipviaSearch.click();
