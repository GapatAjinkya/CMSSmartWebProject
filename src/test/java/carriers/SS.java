...
public class SS {
	...
	// Logger logger = LogManager.getLogger("SS");
	@BeforeClass
	...
	private void addCustomer(String customerName) throws InterruptedException {
		...
		wait.until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//button[@onclick='onCustomerSearchOkClick()']"))).click(); // click
																													// on
																													// ok
		Thread.sleep(5000);
	...
	private void selectSpecialService(String service) throws InterruptedException {
		...
		if (service.equalsIgnoreCase("Hold At Location")) {
			...
			WebElement Address = wait.until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath("//input[contains(@onclick,'ShowCODAddress();')]")));
			wait.until(ExpectedConditions.elementToBeClickable(Address));
			Address.click();
			Thread.sleep(5000);
			wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//button[@id='btncodAddressDataModalOk']")))
					.click();
			Thread.sleep(5000);
			WebElement CompanyNameCOD = wait
					.until(ExpectedConditions.visibilityOfElementLocated(By.id("txtCompanyNameCOD")));
			CompanyNameCOD.clear();
			Thread.sleep(5000);
			CompanyNameCOD.sendKeys("CMS GlobalSoft Org Name");
			WebElement ContactNameCOD = wait
					.until(ExpectedConditions.visibilityOfElementLocated(By.id("txtContactNameCOD")));
			...
			WebElement Save = wait
					.until(ExpectedConditions.visibilityOfElementLocated(By.id("btncodAddressDataModalOk")));
			...
			/*
			 * WebElement selecta = driver.findElement(By.id("SL12")); Select select = new
			 * Select(selecta); select.selectByVisibleText("Signature Required");
			 * Thread.sleep(5000);
			 */
			driver.findElement(By.xpath("//*[@id='btnOk']")).click();
			Thread.sleep(5000);
